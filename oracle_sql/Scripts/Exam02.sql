/*
 * EMPOYEES 테이블의 직원이름, 이메일, 전화번호, 고용일을 조회한다.
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS 직원이름
	 ...
	 , REPLACE (PHONE_NUMBER, '.', '_') AS 전화번호
...
SELECT FIRST_NAME || LAST_NAME AS 직원이름
	 , TO_CHAR(SALARY / 12, '999,999,999.99') AS 급여
	 , TO_CHAR(SALARY, '999,999,999') AS 연봉
  FROM EMPLOYEES;
...
SELECT SUBSTR(PHONE_NUMBER,5,7) AS 전화번호 
	 , PHONE_NUMBER AS 전화번호2
  FROM EMPLOYEES;
/*
