 ...
 * 2. 관리자가 회원 가입을 승인하면 회원 가입 요청 테이블의 데이터를 기반으로 회원 테이블의 데이터가
 *    생성됩니다.
 ...
 * 6. 회원 테이블에 최근 로그인 날짜를 기록하여 로그인을 할 때마다 "n 일 만에 다시 접속하셨습니다."
...
INSERT INTO REQ_ACCOUNTS (ID, NICKNAME, EMAIL)
       VALUES(SEQ_REQ_ACCOUNTS.NEXTVAL, '최가영', 'choi@email.com');
INSERT INTO REQ_ACCOUNTS (ID, NICKNAME, EMAIL)
       VALUES(SEQ_REQ_ACCOUNTS.NEXTVAL, '이정후', 'lee@gmail.com');
INSERT INTO REQ_ACCOUNTS (ID, NICKNAME, EMAIL)
       VALUES(SEQ_REQ_ACCOUNTS.NEXTVAL, '테스트1', 'test1@gmail.com');
INSERT INTO REQ_ACCOUNTS (ID, NICKNAME, EMAIL)
       VALUES(SEQ_REQ_ACCOUNTS.NEXTVAL, '테스트2', 'test2@gmail.com');
...
INSERT INTO REQ_ACCOUNTS (ID, NICKNAME, EMAIL)
       VALUES(SEQ_REQ_ACCOUNTS.NEXTVAL, '테스트3', 'test3@gmail.com');
...
SELECT ID
 ...
 WHERE ISALLOWED = 'P'
   ...
   -- 참고로 TO_DATE('20221201') 은 20221201 00:00:00 기준임.
...
SELECT FLOOR(SYSDATE - TO_DATE(MAX(LOGDATE))) AS N일전접속 
	 , FLOOR(MONTHS_BETWEEN(SYSDATE, MAX(LOGDATE))) AS N개월전접속 
	 , FLOOR(MONTHS_BETWEEN(SYSDATE, MAX(LOGDATE)) / 12) AS N년전접속   
 	 , FLOOR(TO_NUMBER(SYSDATE - MAX(LOGDATE)) * 24) AS N시간전접속
 	 , FLOOR(TO_NUMBER(SYSDATE - MAX(LOGDATE)) * 24 * 60) AS N분전접속
  FROM ACCOUNT_ACCESS_LOGS
 WHERE LOGTYPE = 'LOGOUT';
...
/*
 * 회원이 암호를 변경할 때 현재 사용했던 암호가 새로운 암호에 포함되지 않게 하기위한 구문
 */

CREATE OR REPLACE PROCEDURE PROC_ACCOUNTS(
	   IN_ID IN NUMBER
	 , IN_PASSWORD IN VARCHAR2
)
IS
	VAR_PASSWORD VARCHAR2(70);
BEGIN 
	SELECT PASSWORD INTO VAR_PASSWORD FROM ACCOUNTS WHERE ID = IN_ID;
	...
	IF VAR_PASSWORD <> IN_PASSWORD THEN
		UPDATE ACCOUNTS 
		 ...
		 WHERE ID = IN_ID;
 	ELSE 
 		DBMS_OUTPUT.PUT_LINE('직전 패스워드와 동일한 패스워드는 설정 불가함');
 	END IF;
END;
...
BEGIN
	PROC_ACCOUNTS(2, 'test12345');
END;


SELECT * FROM ACCOUNTS;
SELECT * FROM REQ_ACCOUNTS;
SELECT * FROM ACCOUNT_ACCESS_LOGS ORDER BY ID;

DROP TABLE ACCOUNTS;
DROP TABLE REQ_ACCOUNTS;
DROP TABLE ACCOUNT_ACCESS_LOGS;

DROP SEQUENCE SEQ_ACCOUNTS;
DROP SEQUENCE SEQ_REQ_ACCOUNTS;
DROP SEQUENCE SEQ_ACCOUNT_ACCESS_LOGS;